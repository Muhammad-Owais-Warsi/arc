...
// Generate `IconName` from the icons shipped by the application in
// `assets/icons`. The path is resolved relative to this crate's
// `CARGO_MANIFEST_DIR` (`crates/ui`), reaching the repository
// root's `assets/icons` folder. See `ui_macros::icon_named!`'s
