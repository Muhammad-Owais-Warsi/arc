...
pub struct TabPanel {
    ...
    /// Custom toolbar content for this tab panel. When `None`, a default
    /// toolbar shell is rendered (currently just the layout container with
    /// no buttons). Set this to override with your own content.
    pub(crate) tool_bar: Option<Arc<dyn Fn() -> AnyElement + Send + Sync>>,

    /// Optional button rendered on the right side of the title bar,
    /// intended for actions like adding a new tab.
    pub(crate) add_button: Option<Arc<dyn Fn() -> AnyElement + Send + Sync>>,
...
impl TabPanel {
    ...
    /// Set custom toolbar content, replacing the default toolbar shell's
    /// contents. Persists across renders.
    pub fn set_tool_bar<F>(&mut self, tool_bar: F, cx: &mut Context<Self>)
    where
        F: Fn() -> AnyElement + Send + Sync + 'static,
    {
        self.tool_bar = Some(Arc::new(tool_bar));
        cx.notify();
    }

    /// Set an optional add button rendered on the right side of the title bar.
    /// Useful for adding a "+" button to create new tabs.
    pub fn set_add_button<F>(&mut self, button: F, cx: &mut Context<Self>)
    where
        F: Fn() -> AnyElement + Send + Sync + 'static,
    {
        self.add_button = Some(Arc::new(button));
        cx.notify();
    }
    ...
    /// Renders the toolbar shell (spacing/border/container) with whatever
    /// content is currently set in `self.tool_bar`. Falls back to an empty
    /// container when nothing has been set.
    fn render_toolbar(
        &self,
        _state: &TabState,
        _window: &mut Window,
            ...
            .when(!self.collapsed, |this| {
                this.suffix(
                    h_flex()
                        ...
                        .child(self.render_toolbar(state, window, cx))
                        .when_some(self.add_button.as_ref(), |this, f| {
                            this.child(f())
                        })
