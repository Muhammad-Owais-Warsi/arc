...
impl DockItem {
    ...
    /// Set an add button for the DockItem, only valid for [`DockItem::Tabs`].
    /// The button is rendered on the right side of the tab panel title bar.
    pub fn add_button<F>(mut self, button: F, cx: &mut App) -> Self
    where
        F: Fn() -> AnyElement + Send + Sync + 'static,
    {
        debug_assert!(
            matches!(self, Self::Tabs { .. }),
            "add_button can only be set for DockItem::Tabs"
        );

        if let Self::Tabs { ref mut view, .. } = self {
            view.update(cx, |tab_panel, cx| {
                tab_panel.set_add_button(button, cx);
            });
        }
        self
    }
