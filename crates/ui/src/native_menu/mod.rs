...
mod tests {
    ...
    fn test_native_menu_icon_asset_resolves_to_bytes() {
        ...
        // The app ships its icons under `assets/icons` at the repository root,
        // two levels up from this crate's manifest dir.
        let root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("..")
            .join("..")
            .join("assets");
        let asset_source = TestAssetSource { root };
    ...
    impl AssetSource for TestAssetSource {
        fn load(&self, path: &str) -> gpui::Result<Option<Cow<'static, [u8]>>> {
            Ok(self.root.join(path).exists().then(|| {
                Cow::Owned(std::fs::read(self.root.join(path)).unwrap())
            }))
        }

        fn list(&self, path: &str) -> gpui::Result<Vec<SharedString>> {
            let full_path = self.root.join(path);
            Ok(if full_path.is_dir() {
                std::fs::read_dir(&full_path)
                    .unwrap()
                    .map(|entry| {
                        SharedString::from(entry.unwrap().file_name().to_string_lossy().to_string())
                    })
                    .collect()
            } else {
                Vec::new()
