...
///   The `IconName` enum in `ui` uses this pattern to consume the
///   icons shipped by the application under `assets/icons`.
...
///   through cargo's `links` / `DEP_<X>_<KEY>` propagation mechanism.
///
/// # Example
///
/// ```ignore
...
/// icon_named!(IconName, "$UI_DEFAULT_ICONS_DIR");
///
/// // With custom derives
