...
use gpui::*;
use ui::dock::{Panel, PanelControl, PanelEvent};
use ui::input::{Input, InputEvent, InputState};
use ui::sidebar::{SidebarItem, SidebarMenu, SidebarMenuItem};
...
impl Panel for ProjectPanel {
    ...
    fn title(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        div().child(self.get_selected_workspace().to_string())
    }
...
impl Render for ProjectPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let ws = &self.workspaces[self.selected_workspace];
        let menu = SidebarMenu::new().children(ws.root_id.iter().map(|&id| self.render_node(id, cx)));
        div().size_full().child(menu.render("api-tree", window, cx))
    }
}
